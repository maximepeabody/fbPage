...
.factory('page', ['Facebook', function(Facebook) {
    ...
    page.loadMainContent = function() {
        Facebook.api(page.pageId + '?fields=cover,name,feed,phone,about,emails,current_location,description,company_overview,hours,price_range,photos{picture,album}',{access_token: page.token}, function(response){
            console.log(page);
    ...
    page.getFeed = function() {
        Facebook.api(page.pageId + '?fields=feed.limit(10)', {access_token:page.token}, function(response) {
            page.feed = response.feed;
    ...
    page.getProfilePicture = function(id) {
        Facebook.api(id, {access_token: page.token}, function(response) {
    ...
    page.getNextPage= function(){
        Facebook.api(page.feed.paging.next.substring(31), {access_token:page.token}, function(response) {
            console.log(response);
            page.feed = response;
            for(var i = 0; i< page.feed.data.length; i++) {
                page.setObject(page.feed.data[i].object_id, page.feed.data[i].status_type, i);
    ...
    page.getPreviousPage= function(){
        Facebook.api(page.feed.paging.previous.split(31), {access_token:page.token}, function(response) {
            page.feed = response.feed;
            console.log(response);
        })
    };
    ...
    page.setCover = function(coverId) {
         Facebook.api(coverId, {access_token:page.token}, function(response) {
    ...
    page.setStory  = function(object_id, index){
        if(object_id) {
            Facebook.api(object_id, {access_token:page.token}, function(response){
    ...
    page.setEvent = function(object_id, index) {
        if(object_id){
            Facebook.api(object_id + '?fields=cover,description', {access_token:page.token}, function(response){
...
.controller('appCtrl', ['$scope','page', 'Facebook',function($scope,page, Facebook){
